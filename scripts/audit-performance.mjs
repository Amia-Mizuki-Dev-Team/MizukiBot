...
const transparentAvatar = resolve(DIST_DIR, 'Picture/avatar-transparent.svg')
if (!existsSync(transparentAvatar)) {
  errors.push('构建输出缺少透明首页头像资源')
} else {
  const svg = readFileSync(transparentAvatar, 'utf8')
  if (!svg.includes('<clipPath') || !svg.includes('/Picture/avatar.jpg')) {
    errors.push('透明首页头像资源缺少人物轮廓裁剪或原图引用')
  }
}
